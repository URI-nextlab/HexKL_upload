...
int main() {
  ...
  uint8_t* X_ui8_cpu = NULL;
  int8_t* W_i8_cpu   = NULL;
  uint8_t* W_ui8_npu = NULL;

  size_t A_i32_cpu_size = N_ROW * N_COL * sizeof(*A_i32_cpu_reference);
  size_t X_u8_cpu_size  = N_ROW * N_INNER * sizeof(*X_ui8_cpu);
  size_t W_i8_cpu_size  = N_COL * N_INNER * sizeof(*W_i8_cpu);
  size_t W_i4_size =
    ((n_col_32 * n_inner_32) % 2 ? (n_col_32 * n_inner_32 + 1) : (n_col_32 * n_inner_32)) / 2 * sizeof(*W_ui8_npu);
  ...
  A_i32_npu           = malloc(A_i32_cpu_size);
  X_ui8_cpu           = malloc(X_u8_cpu_size);
  W_i8_cpu            = malloc(W_i8_cpu_size);

  SDKL_CHECK(sdkl_npu_alloc(W_i4_size, (void**)&W_ui8_npu));
  ...
  for (size_t i = 0; i < N_ROW; i++) {
    for (size_t j = 0; j < N_INNER; j++) {
      X_ui8_cpu[i * N_INNER + j] = (uint8_t)rand() % 127;
      rand();
    }
  }
  ...
  for (size_t i = 0; i < N_COL; i++) {
    for (size_t j = 0; j < N_INNER; j++) {
      rand();
      W_i8_cpu[i * N_INNER + j] = (int8_t)(rand() % 16) - 8;
    }
  }
  ...
  matmul_ui8i8_i32_rm(N_ROW, N_COL, N_INNER, A_i32_cpu_reference, X_ui8_cpu, W_i8_cpu);
  ...
  gettimeofday(&start, NULL);
  SDKL_CHECK(sdkl_cpu_rm_to_wh_i4(W_ui8_npu, W_i8_cpu, n_inner_32, n_col_32));
  gettimeofday(&end, NULL);
  ...
  gettimeofday(&start, NULL);
  SDKL_CHECK(sdkl_npu_mm_u8i4_i32(domain, N_ROW, N_COL, N_INNER, A_i32_npu, X_ui8_cpu, W_ui8_npu));
  gettimeofday(&end, NULL);
  ...
  SDKL_CHECK(sdkl_npu_free(W_ui8_npu));
