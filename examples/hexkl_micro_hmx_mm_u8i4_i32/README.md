...
Simple Test for `hexkl_micro.a` API: `hexkl_micro_hmx_mm_u8i4_i32`

Overview
--------
This project provides a minimal test harness for the `hexkl_micro.a` library, specifically targeting matrix multiplication using the HMX engine via the micro API with 4-bit quantized weights.
...
The test demonstrates usage of the following API functions:
  ...
  int hexkl_micro_hmx_mm_u8i4
  int hexkl_micro_hmx_acc_read_int32
  int hexkl_micro_hmx_copy_32b_to_submatrix

These functions collectively demonstrate initialization, memory layout conversion, matrix multiplication with 4-bit weights, and result retrieval using the HMX engine.
